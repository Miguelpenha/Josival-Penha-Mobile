...
async function generateDocument(student: string, frequency: number, isScholarship: boolean, name: String) {
  const URLDocument = `${process.env.EXPO_PUBLIC_API_URL}/students/documents/declaration/${student}?frequencyPercentage=${frequency}&scholarshipStudent=${isScholarship}`
